...
const COLUMN_MAPPING = {
  ...
  'lt.value:category': 'item_category',
  'it.value:category': 'item_category',
  'lt_value_category': 'item_category'
};
...
function parseExcel(filePath) {
  ...
  workbook.SheetNames.forEach(sheetName => {
    ...
    // Process any tab that contains the word "rating" or "feedback"
    if (normalizedSheetName.includes('rating') || normalizedSheetName.includes('feedback')) {
      ...
      rawRows.forEach(row => {
        ...
        // Deduplicate using a strict date format and order_id as a unique key
        const safeDate = normalizeDateString(cleanedRow.date) || 'nodate';
        ...
        const dedupeKey = `${safeDate}_${safeOrderId}_${safeRestId}_${safeItemName}`;
